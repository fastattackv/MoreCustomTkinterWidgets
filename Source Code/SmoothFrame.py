...
def get_coordinates_from_grid(master, column: int, row: int) -> tuple[int, int]:
    ...
    temp_frame.grid(row=row, column=column, sticky="nw")
...
class SmoothFrame(ctk.CTkFrame):
    ...
    def smooth_place(self, time_to_move: float, x: int, y: int, frequency=120):
    ...
    def smooth_grid(self, time_to_move: float, column: int, row: int, columnspan=1, rowspan=1, sticky="", padx=0, pady=0, ipadx=0, ipady=0, frequency=120):
        ...
        if self.grid_info():  # has been gridded
            ...
            x, y = get_coordinates_from_grid(self.master, column, row)
